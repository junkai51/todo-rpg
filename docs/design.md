...
先实现可用的现实记录工具，电脑浏览器优先，兼顾 iPhone。当前 Reality Domain 初步版本实现 Todo + Routine；Habit 仅保留设计。GM、Mock GM、奖励预算、GameState、Quest、World Chat 和世界生成不属于当前实现。
...
| Routine | 根据 `schedule` 提供可执行时间窗口的例行事项 | 定义持续存在，每个有效窗口分别完成；可停用／启用 | 实现每天、每 N 天、指定星期 |
...
- `schedule` 表示 occurrence 的计划窗口，不是 deadline。当前支持每天、每 N 天和每周指定的一个或多个星期，附开始日期。
- 每 N 天的 N 为 1–365 的整数，以 `startDate` 为固定起点：起点当天、起点 + N 天、起点 + 2N 天依次提供窗口。按日历日期差计算，跨月、闰年及夏令时不改变节奏；完成、漏做和 Turn 确认不重置起点。
- 对应日程字段为 `frequency: 'interval'`、`intervalDays: N`，与 `daily`、`weekly` 并列。编辑开始日期或 N 会从保存时刻生效并保留日程历史，旧完成快照不变。
...
本次里程碑为 Reality Domain 初步完成：Todo／Routine、独立日程窗口、有效事件提交、Today、日程管理、持久化和开发调试已形成可运行闭环。GM 接入前仍需另外决定 progression 规则；不把这个里程碑视为整个产品完成。

本次添加日程类型沿用 v4 数据格式，已有 v4 记录不会清空；旧版本的一次性清空规则仍只作用于 v1–v3。

当前验收重点：Todo 完成与撤销的最终有效状态、Routine 当前窗口／跨日／指定星期／过期拒绝、完成和 Turn 独立、提交后清理、事件类型区分、快照隔离、并发幂等、一次性旧格式清空、手动清空与重开、桌面和窄屏操作、正式构建隐藏调试。

暂不实现 Habit UI、复杂 RRULE、月／年重复、时段窗口、节假日、提醒、日历视图、Project、子任务、时间追踪、统计、已确认历史纠错或跨设备同步。
...
| 2026-09-15 | Routine schedule 独立于 Turn，只提供有效时间窗口；当时仅实现每天和指定星期。 |
| 2026-09-15 | 两类完成生成不同 RealityEvent；确认后已完成项从页面清除，历史仍保留。 |
| 2026-09-15 | 调试改为可视卡片；用户允许清空旧记录，并要求一键清空功能。 |
| 2026-09-15 | 恢复 Routine 每 N 天日程，按开始日期保持节奏；发布 Reality Domain 初步完成版本。 |
